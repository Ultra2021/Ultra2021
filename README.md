<h1 align="center">✨ Hey, I'm Noel Biju ✨</h1>

<p align="center">
  <em>Developer • Dreamer • Digital Craftsman</em><br>
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=22&color=00FF00&center=true&vCenter=true&width=500&lines=Code.+Create.+Repeat.; Building+cool+things+with+logic+and+art.;Turning+ideas+into+reality." />
</p>

---

### 🎨 About Me

I’m not just writing code — I’m designing experiences. Whether it’s a sleek web app, a tight algorithm, or a perfectly tuned C loop, I love finding the rhythm in logic.

- 🌱 Learning: **Full-stack dev**, **C++**, and **AI concepts**
- 🔭 Working on: Passion projects that blend **creativity + engineering**
- 💬 Ask me about: **Programming**, **Digital Electronics**, or **Clean Energy Technologies**
- 🎯 Goal: To keep building things that *matter* and look *great*
- ⚡ Fun Fact: I debug best at midnight with a playlist and caffeine

---

### 🧠 Tech Toolbox

| 💻 Languages | 🧰 Tools | 🌐 Concepts |
|--------------|----------|-------------|
| C, C++, Java, Python | VS Code, Git, Bootstrap, Node.js | OOP, Logic Circuits, Clean Code |

---

### 🚀 Featured Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Ultra2021&show_icons=true&theme=radical" height="170" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Ultra2021&layout=compact&theme=radical" height="170" />
</p>

---

### 🌀 Current Vibes

<p align="center">
  <img src="https://github.com/Platane/snk/raw/output/github-contribution-grid-snake.svg" alt="snake eating my contributions" />
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Ultra2021&label=Visitors&color=blue&style=flat-square" alt="visitor badge"/>
</p>

---

### 🧩 Quote to Code By

> “Great code isn’t written. It’s composed — like music for machines.”  
> — Noel Biju 🎧
